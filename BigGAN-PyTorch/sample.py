...
def run(config):
  ...
  for id in ids:
    for i in range(50):
      ...
      y = [id]
      z = torch.FloatTensor(z).to(device, torch.float32)
      y = torch.FloatTensor(y).to(device, torch.int64)
      ...
      print(id, i)
      imageio.imwrite('./samples/%d/%.2d.jpg' % (id,i), x) 
      np.save('./samples/%d/%.2d_Z.npy' % (id,i), z.detach().cpu().numpy())  
  ...
  if config['sample_npz']:
    ...
    for i in trange(int(np.ceil(config['sample_num_npz'] / float(G_batch_size)))):
      ...
      x = np.transpose(x[0,...], (1, 2, 0))
      imageio.imwrite('./samples/%d.jpg' % i, x) 
